...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("nextUntil() to get all of the next sibling DOM elements within elements until another element", () => {
    cy.get('.traversal-drinks-list').contains('Coffee').nextUntil('#milk') //returns until milk, not included
  });

  it("not() to remove DOM element(s) from the set of elements", () => {
    cy.get('.traversal-button-states > button').not('.disabled').should('not.have.class', 'disabled') //element not to included
  });
  ...
  it("prev() to get the previous sibling DOM element within elements", () => {
    cy.get('.traversal-drinks-list').contains('Sugar').prev().should('contain', 'Espresso')
  });
